...
public class SimplePaint extends JApplet {
   ...
   public static class SimplePaintPanel extends JPanel
      ...
      private Graphics graphicsForDrawing2;
      DrawBoard drawboard;
      int xg=0;
      int yg=0;
      ...
      public void mousePressed(MouseEvent evt) {
         ...
         else if (x > 3 && x < width - 3 && y > 3 && y < height - 3) {
      ...
      public void mouseDragged(MouseEvent evt) {
    	  ...
    	  Graphics2D g1 = (Graphics2D) graphicsForDrawing;
         if (dragging == false)
            return;  // Nothing to do because the user isn't drawing.
         ...
         if((x> getWidth()-57) && (y> getHeight()-54)){
        	 x=getWidth()-57;
        	 y=getHeight()-54;
         }
         
         g1.setStroke(new BasicStroke(3));
         g1.drawLine(prevX, prevY, x, y);  // Draw the line.
         xg=x;
         yg=y;
         drawboard.run();
        // System.out.println(graphicsForDrawing.getColor().getRGB());
      ...
      public class DrawBoard {
    		...
    		/**
    		 * Send and receive simple co-ordinates as messages. This sample uses meta fields from
    		 * a message to send strings.
    		 */
    		
    		public Discovery disc;
    		public Node drawNode;
    		public Subscriber drawSub;
    		public Publisher drawPub;
    		...
    		public DrawBoard(SimplePaintPanel sp) {
    			this.sp=sp;
    			disc = new Discovery(DiscoveryType.MDNS);
    			
    			drawNode = new Node();
    			drawSub = new Subscriber("drawBoard");
    			drawSub.setReceiver(new DrawReceiver());
    			
    			drawPub = new Publisher("drawBoard");
    			...
    			drawPub.setGreeter(new DrawGreeter(userName));
    			drawNode.addPublisher(drawPub);
    			drawNode.addSubscriber(drawSub);
    		}
    		 
    		class DrawReceiver extends Receiver {
    			
    			@Override
    			public void receive(Message msg) {
    				
    				if (msg.getMeta().containsKey("xval")) {
    					xglob= Integer.parseInt(msg.getMeta("xval"));
    				}
    				if (msg.getMeta().containsKey("yval")) {
    					yglob= Integer.parseInt(msg.getMeta("yval"));
    				}
    				
    				if((xglob==0 && yglob==0)||(prevxglob==0 && prevyglob==0)){
    				}
    				else{
    					if(sp!=null){
        				sp.graphicsForDrawing2=getGraphics();
        				graphicsForDrawing2.setColor(Color.RED);
        				Graphics2D g = (Graphics2D) graphicsForDrawing2;
        				g.setStroke(new BasicStroke(3));
        				if((Math.abs(prevxglob-xglob)<25) && (Math.abs(prevyglob-yglob)<25))
    						g.drawLine(prevxglob, prevyglob, xglob, yglob);
    		...
    		class DrawGreeter extends Greeter {
    			public String userName;

    			public DrawGreeter(String userName) {
    			...
    			public void welcome(Publisher pub, SubscriberStub subStub) {
    				...
    				greeting.putMeta("subscriber", DrawBoard.this.drawSub.getUUID());
    				pub.send(greeting);
    			}

    			@Override
    			public void farewell(Publisher pub, SubscriberStub subStub) {
    				if (DrawBoard.this.participants.containsKey(subStub.getUUID())) {
    					System.out.println(DrawBoard.this.participants.get(subStub.getUUID()) + " left the board");
    				} else {
    					System.out.println("An unknown user left the board: " + subStub.getUUID());	
    				}
    			}	
    		}
    		...
    		public void run() {
    			...
    			msg.putMeta("xval", Integer.toString(xg));
    			msg.putMeta("yval", Integer.toString(yg));
